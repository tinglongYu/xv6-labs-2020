...
# 包含一个名为 lab.mk 的文件，该文件的内容是 LAB=util。这意味着它会引入 lab.mk 文件中定义的变量和规则，并将其合并到当前的Makefile中。
-include conf/lab.mk
...
# 检查一个名为 LAB 的变量是否等于 "pgtbl"。如果 LAB 的值等于 "pgtbl"，那么它会将目标文件 $K/vmcopyin.o 添加到 OBJS 变量中。
ifeq ($(LAB),pgtbl)
OBJS += $K/vmcopyin.o
endif
...
# 定义编译选项，其中
# -Wall: 这个选项告诉编译器启用所有常见的警告 
# -Werror: 这个选项将所有警告视为错误
# -O: 这是优化选项，用于启用编译器的优化功能。
...
# fs.img 是文件系统的镜像，其中包含了用户程序和其他文件系统的相关信息。
# fs.img 将会包含 $(UEXTRA) 中的文件以及 $(UPROGS) 中指定的用户程序。
# 这些用户程序是通过 $(UPROGS) 指定的可执行文件构建的。
# 所以，$(UPROGS) 中指定的可执行文件会在构建 fs.img 时被编译，然后包含在文件系统镜像中，以供操作系统在运行时使用。
